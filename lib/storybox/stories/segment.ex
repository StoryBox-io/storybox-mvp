defmodule Storybox.Stories.Segment do
  ...
  def pin_target_latest_version(%{pin_type: :synopsis_piece, pin_id: pin_id})
    ...
    Storybox.Stories.SynopsisPiece
    |> Ash.Query.filter(sequence_id == ^pinned.sequence_id)
  ...
  def pin_target_latest_version(%{pin_type: :sequence_piece, pin_id: pin_id})
      when not is_nil(pin_id) do
    pinned =
  ...
  def pin_target_latest_version(%{pin_type: :sequence_vv, pin_id: pin_id})
      when not is_nil(pin_id) do
    pinned =
    ...
    Storybox.Stories.SequenceViewVersion
    |> Ash.Query.filter(sequence_view_id == ^pinned.sequence_view_id)
    |> Ash.read!(authorize?: false)
    |> Enum.map(& &1.version_number)
    |> Enum.max(fn -> nil end)
  end
